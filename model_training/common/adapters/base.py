...
class ModelAdapter:
    def __init__(self, config, log_path):
        ...
        self.criterion = self.get_loss_function(config['model']['loss'])
    ...
    def get_loss_function(self, losses_config):
        if isinstance(losses_config, dict):
            return {losses_config['name']: (
                torch.nn.DataParallel(get_loss(losses_config).to(self.device), device_ids=self.devices),
                1.0
            )}
        elif isinstance(losses_config, list):
            return {x['name']:
                (
                    torch.nn.DataParallel(get_loss(x).to(self.device), device_ids=self.devices),
                    x.get('weight', 1.0)
                )
    ...
    def get_loss(self, y_pred, data):
        ...
        for criterion, weight in self.criterion.values():
            loss += weight * torch.mean(criterion(y_pred, y_true))
        return loss
