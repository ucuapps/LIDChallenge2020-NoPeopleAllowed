...
from .irnet_resnet import IRNet
...
def get_network(model_config):
    ...
    if arch == "deeplabv3_resnet50":
        ...
        return resnet50_full(
            pretrained=model_config["pretrained"],
            num_classes=model_config["classes"],
            weights_path=model_config.get("weights_path", None),
        )
    elif arch == "resnet50_dropblock":
        return resnet50_dropblock(
            num_classes=model_config["classes"],
            pretrained=model_config["pretrained"],
        ...
        return resnet50_dropblock_scheduler(
            num_classes=model_config["classes"],
            pretrained=model_config["pretrained"],
            drop_prob=model_config["regularize"]["parameters"]["drop_prob"],
            block_size=model_config["regularize"]["parameters"]["block_size"],
        ...
        return UNet(
            encoder=model_config["encoder"], num_classes=model_config["classes"]
        )
    elif arch == "IRNet":
        return IRNet(freeze_backbone=model_config["freeze_backbone"])
    else:
