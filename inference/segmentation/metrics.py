...
class IoUNumpy(Metric):
    ...
    def add(self, output, target):
        ...
        for i, j in itertools.product(np.unique(target), np.unique(output)):
            self.conf_matrix[i, j] += np.sum((target[valid_idx] == i) & (output[valid_idx] == j))
    ...
    def get(self):
        ...
        metric_per_class = true_positives / (true_positives + false_negatives + false_positives)
        return np.nanmean(metric_per_class), metric_per_class
